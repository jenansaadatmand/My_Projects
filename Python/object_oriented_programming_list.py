...
# Each time you create a list, your list is created based on Python's built-in class Array, which inherits all behaviours or methods from the array. Similarly, when you create a Python list you inherit all methods from list class.  This ensures that objects only have methods that they need to avoid needlesly overload computer memory with unnecessary extra methods, at the sametime, this OOP to organization method allows for easily creating new data objects and immediately using most common methods to work with that data, OOP frees us up from repetatively building the same basic building blocks, more time to build the functionality and features we want in the program 
